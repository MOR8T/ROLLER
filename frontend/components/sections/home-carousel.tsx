 ...
 * steps from a row of dots and arrows under the strip — `ui/slider-pager.tsx`,
 * the same control row the hero deck and the product page's gallery carry, so
 * every slider on the site is driven and read the same way.
 *
 * ── Why the beat is not Swiper's ────────────────────────────────────────────
 *
 * The pager draws the countdown to the next turn, and a bar is a promise: it
 * says the strip moves when the fill reaches the end. Swiper's `Autoplay`
 * keeps that clock inside itself — its pause on hover, its own idea of what a
 * drag should do to the delay — so the bar would have been a second animation
 * running alongside the real one and agreeing with it only by luck.
 *
 * So the module is gone and `lib/use-autoplay.ts` calls `slideNext` instead:
 * one clock, driving the strip and drawing itself. `pauseOnMouseEnter` is the
 * pointer handlers on the wrapper below, and a drag or an arrow restarts the
 * beat rather than letting the new slide inherit the tail of the old one.
 * Everything else about the strip — the loop, the buffer, the touch angle — is
 * still Swiper's and is unchanged.
 *
...
export function HomeCarousel({
  ...
  const { beat, setPaused, restart } = useAutoplay({
    ...
    // The strip loops, so this never reaches an end to stop against. `slideNext`
    // is what the right-hand arrow already calls.
    onAdvance: () => swiper?.slideNext(),
  });

  /** A turn the visitor asked for: take it, then start the beat over. */
  const ask = (turn: () => void) => {
    turn();
    restart();
  };
  ...
  return (
    <div className={cn("relative", className)}>
      {/* The wrapper is what `pauseOnMouseEnter` used to be, and it stops at the
          strip: held on the outer element it would cover the pager too, so the
          pointer that pressed an arrow would also be the one holding the clock,
          and the bar it had just restarted would sit frozen at zero.

          Focus pauses as well. A keyboard visitor reading a headline inside the
          strip has no pointer to hover with, and a card sliding out from under
          the link they are about to press is the worst version of this.

          ⚠️ These handlers cannot go on `<Swiper>` itself: swiper-react reads
          any `on[A-Z]` function prop as the name of a *Swiper* event, so
          `onPointerEnter` there would quietly become an event called
          `pointerEnter` and never reach the DOM. */}
      <div
      ...
      >
        <Swiper
          ...
          // A drag is a request to move on. Swiper's own autoplay would have
          // resumed the tail of the interrupted beat here; the strip starts a
          // fresh one instead, so the bar the visitor is looking at after their
          // swipe measures the slide they landed on.
          onTouchEnd={restart}
          modules={[A11y]}
      ...
      {/* The strip keeps playing after a drag and after an arrow — stopping for
          good on first touch is what makes an autoplaying carousel look broken
          the moment anyone brushes it. `slideToLoop` takes a real index and
          finds whichever copy of it is nearest, so a dot stays one short hop
          away even on a strip whose list is repeated. */}
      <SliderPager
        count={slides.length}
        active={active}
        onSelect={(index) => ask(() => swiper?.slideToLoop(index))}
        onPrevious={() => ask(() => swiper?.slidePrev())}
        onNext={() => ask(() => swiper?.slideNext())}
        labels={{
          previous: t("previous"),
          next: t("next"),
