...
Работа идёт в `dev`, вливается PR'ом в `development`. Сборка запускается на
пуш в `development` и `main`. Сервер следует за тегом `development`
(`IMAGE_TAG` в `.env`).
...
| `<имя ветки>` | всегда | движется вместе с веткой |
| `latest` | только `main` | боевой |
...
| `IMAGE_TAG` | какую сборку запускать. **Обязательна** |
...
**Порядок такой:** сначала PR влит в `development`, затем сборка на вкладке
...
- [ ] **`main` актуален** и `IMAGE_TAG` переключён на `latest`.
