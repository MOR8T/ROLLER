...
| `INTERNAL_API_TOKEN` | общий секрет бэкенда и Next (`openssl rand -hex 32`). **Обязательна** |
| `INITIAL_ADMIN_*` | первый администратор, создаётся один раз на пустой базе |

`INTERNAL_API_TOKEN` — единственное, что защищает `GET /api/site-settings/preview-code`:
Next читает по нему код доступа к закрытому сайту и сверяет его у себя, а не
гоняет каждую проверку в FastAPI. Значение должно совпадать у обоих сервисов;
если оно не задано, маршрут отключён (503) и превью закрытого сайта просто не
работает — сайт остаётся закрытым, что и есть правильное направление отказа.
Наружу этот путь не выставлен: `nginx/includes/app-locations.inc` глушит его
`deny all`, а контейнеры ходят друг к другу по внутренней сети Compose.

`IMAGE_TAG`, `PUBLIC_ORIGIN` и `INTERNAL_API_TOKEN` намеренно сделаны обязательными. Значение по
